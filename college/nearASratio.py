...
def process_ASratio(ID, timerange, interval, windowsize_FFT):
    '''
    指定された ID に対応する MUTC (ダストデビル発生時刻)直前の
    時系列データにおける気圧残差を求め、
    それに対する振幅スペクトルとその移動平均の比(振幅スペクトル比)を算出する関数。
    ...
    try:
        ...
        return  moving_AS_x, ratio, sol

    except Exception as e:
        print(f"An error occurred: {e}")
        return None

def plot_ASratio(ID, timerange, interval, windowsize_FFT):
    '''
    指定された ID に対応する MUTC (ダストデビル発生時刻)直前の
    時系列データにおける気圧残差を求め、
    それに対する振幅スペクトル比を算出し、プロットを保存する関数。
    ...
    try:
        # 振幅スペクトル比の導出
        moving_AS_x, ratio, sol = process_ASratio(ID, timerange, interval, windowsize_FFT)

        # 特定の周波数より高周波の情報をnanに変更
        #moving_AS_x, ratio = nearratio.filter_xUlimit(moving_AS_x, ratio, 0.8)
        ...
        plt.yscale('log')
        plt.ylim(1e-8,1e2)
        plt.plot(moving_AS_x, ratio, label='ratio')
        ...
        # 保存の設定
        output_dir = f'nearASratio_{timerange}s_windowsize_FFT={windowsize_FFT}'
...
if __name__ == "__main__":
    ...
    plot_ASratio(args.ID, args.timerange, 20, args.windowsize_FFT)