...
def after_movingAS(data, windowsize_FFT):
    """
    ...
    # 振幅スペクトルの導出
    AS_x, AS_y = afterAS.after_AS(data)
...
def process_aftermovingAS(ID, timerange, interval, windowsize_FFT):
    '''
    ...
    try:
        ...
        AS_x, AS_y, moving_AS_x, moving_AS_y = after_movingAS(after_devildata, windowsize_FFT)
...
def plot_aftermovingAS(ID, timerange, interval, windowsize_FFT):
    '''
    指定された ID に対応する MUTC (ダストデビル発生時刻)直後の
    時系列データにおける気圧残差を求め、
    ...
    try:
        # 振幅スペクトルとその移動平均導出
        AS_x, AS_y, moving_AS_x, moving_AS_y, sol = process_aftermovingAS(ID, timerange, interval, windowsize_FFT)
        ...
        #保存の設定
        output_dir = f'aftermovingAS_{timerange}s_windowsize_FFT={windowsize_FFT}'
...
if __name__ == "__main__":
    ...
    plot_aftermovingAS(args.ID, args.timerange, 20, args.windowsize_FFT)